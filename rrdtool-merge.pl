...
#ERROR: usage rrdtcol restore [--range-check/-r] [--force-overwrite/-f] file.xml file.rrd

`$optctl{rrdtool} restore --range-check --force-overwrite $temp_xml $optctl{mergedrrd}`;
